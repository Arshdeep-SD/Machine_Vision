...
int main( int argc, char **argv )
{
	...
	for ( k = 0; k < 3; k ++)
	{
		...
		for (i=0; i<ROWS; i++)
		{
		  for (j=0; j<COLUMNS; j++)
		  {
			bimage[i][j] = (image[i][j] < threshold[k]) ? 255 : 0;
			
		    A += (bimage[i][j] == 255) ? 1 : 0;
			
			xbar += i * ((bimage[i][j] == 255) ? 1 : 0);
			ybar += j * ((bimage[i][j] == 255) ? 1 : 0);
		  }
		}
		...
		for (i=1; i<=5; i++)
		{
		  bimage[(int)xbar+i][(int)ybar] = 128;
		  bimage[(int)xbar-i][(int)ybar] = 128;
		  bimage[(int)xbar][(int)ybar-i] = 128;
		  bimage[(int)xbar][(int)ybar+i] = 128;
		}
		...
		printf("%s: %d %d %d %d\n", ifile, threshold[k], A, (int)xbar, (512-(int)ybar));
